...
- **表冠旋转**：缩放棋盘（替代双指捏合）——向后放大至局部精准落子，向前缩小可将 15x15 整盘收进圆屏（四个角也完整可见）。
...
本应用以 **GPL-3.0** 发布（全文见 `LICENSE`），© 2026 Qwara-chan。之所以是 GPL-3.0：打包的 Rapfi
引擎本身是 GPL-3.0，链接它的分发必须采用同一许可。
